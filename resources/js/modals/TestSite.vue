  ...
  <modal name="test-site"
      ...
      >
        ...
        <div class="content">
          ...
          <md-card-content>
              <label>Kỳ thi</label>
              <select class="select-css" v-model="selectedTestId">
...
export default {
  ...
  methods: {
    beforeOpen (event) {
      ...
      if(event.params.testSiteId) {
        ...
        rf.getRequest('TestSiteRequest').show(this.editingId).then((testSite)=>{
          ...
          this.selectedRoomId = testSite.exam_id;
          this.selectedExamSessionId = testSite.exam_session_id;
        });
      }
    },
    beforeClose() {
    ...
    updateOneTestSite() {
      ...
      params.exam_id = this.selectedExamId;
  ...
  created () {
    rf.getRequest('TestSiteRequest').getAllExamSessions().then((examSessions)=>{
      this.examSessions = examSessions;
    });

    rf.getRequest('TestSiteRequest').getAllExams().then((exams)=>{
      this.exams = exams;
    });
  }
}
